      ...
      <main className="flex-grow">
        <section className="container mx-auto px-4 py-12">
          <div className="relative overflow-hidden rounded-[2rem] mb-10 h-[340px] shadow-xl">
            ...
            >
              {[
                'https://images.unsplash.com/photo-1498079022511-d15614cb1c02?auto=format&fit=crop&w=1400&q=80',
            ...
            <div className="absolute inset-0 z-10 flex flex-col justify-center p-10 text-white">
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-3xl bg-white/10 p-5 shadow-lg backdrop-blur-sm">
                  <p className="text-sm uppercase tracking-[0.3em] text-white/70 mb-2">Posts</p>
                  <p className="text-3xl font-semibold">{totalPosts}</p>
                  <p className="text-sm text-white/70 mt-2">Published stories</p>
                </div>
                <div className="rounded-3xl bg-white/10 p-5 shadow-lg backdrop-blur-sm">
                  <p className="text-sm uppercase tracking-[0.3em] text-white/70 mb-2">Categories</p>
                  <p className="text-3xl font-semibold">{totalCategories}</p>
                  <p className="text-sm text-white/70 mt-2">Topic buckets</p>
                </div>
                <div className="rounded-3xl bg-white/10 p-5 shadow-lg backdrop-blur-sm">
                  <p className="text-sm uppercase tracking-[0.3em] text-white/70 mb-2">Popular</p>
                  <p className="text-3xl font-semibold">{mostViewed?.view_count || 0}</p>
                  <p className="text-sm text-white/70 mt-2">Top reads</p>
                </div>
                <div className="rounded-3xl bg-white/10 p-5 shadow-lg backdrop-blur-sm">
              ...
              <div className="flex flex-wrap items-center gap-3">
                {categoryList.map((category) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-semibold transition ${selectedCategory === category ? 'bg-secondary text-white' : 'bg-white/10 text-white/70 hover:bg-white/20'}`}
                  >
                ...
                {otherPosts.map((post) => (
                  <article key={post.post_id} className="card overflow-hidden shadow-lg">
                    ...
                    <div className="p-6">
                      ...
                      <p className="text-white/50 leading-relaxed mb-5">{getExcerpt(post)}</p>
