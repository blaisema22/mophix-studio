            ...
            <div className="absolute inset-0 z-10 flex flex-col justify-center p-10 text-white">
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-3xl bg-white/10 p-5 shadow-lg backdrop-blur-sm">
                  <p className="text-sm uppercase tracking-[0.3em] text-white/70 mb-2">Galleries</p>
                  <p className="text-3xl font-semibold">{galleries.length}</p>
                  <p className="text-sm text-white/70 mt-2">Curated collections</p>
                </div>
                <div className="rounded-3xl bg-white/10 p-5 shadow-lg backdrop-blur-sm">
                  <p className="text-sm uppercase tracking-[0.3em] text-white/70 mb-2">Published</p>
                  <p className="text-3xl font-semibold">{publishedCount}</p>
                  <p className="text-sm text-white/70 mt-2">Published sets</p>
                </div>
                <div className="rounded-3xl bg-white/10 p-5 shadow-lg backdrop-blur-sm">
                  <p className="text-sm uppercase tracking-[0.3em] text-white/70 mb-2">Photos</p>
                  <p className="text-3xl font-semibold">{totalPhotos}</p>
                  <p className="text-sm text-white/70 mt-2">Images available</p>
                </div>
                <div className="rounded-3xl bg-white/10 p-5 shadow-lg backdrop-blur-sm">
                  <p className="text-sm uppercase tracking-[0.3em] text-white/70 mb-2">Latest</p>
                  <p className="text-3xl font-semibold">{latestPublished ? formatDate(latestPublished) : '—'}</p>
                  <p className="text-sm text-white/70 mt-2">Newest release</p>
                </div>
              </div>
            </div>
          </div>

          <div className="mb-10 rounded-[2rem] border border-orange-400/20 bg-black-dark p-6 shadow-lg">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-sm uppercase tracking-[0.4em] text-orange-400 mb-3">Portfolio overview</p>
                <h2 className="section-title text-white">Capture the full story behind every shoot</h2>
                <p className="section-subtitle max-w-2xl text-white/50">
              ...
              <div className="flex flex-wrap items-center gap-3">
                {eventTypes.map((type) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-semibold transition ${selectedType === type ? 'bg-orange-400 text-black' : 'bg-white/10 text-white/70 hover:bg-white/20'}`}
                  >
                    {type === 'all' ? 'All' : type}
                  </button>
                ))}
          ...
          {loading ? (
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {visibleGalleries.map((gallery) => (
                <article key={gallery.gallery_id} className="rounded-[1.75rem] border border-orange-400/20 bg-black-dark overflow-hidden shadow-xl">
                  ...
                  <div className="p-6">
                    <div className="mb-3 flex flex-wrap items-center gap-2">
                      <span className="text-sm uppercase tracking-[0.3em] text-orange-400">{gallery.photo_count || 0} photos</span>
                      <span className="text-sm uppercase tracking-[0.3em] text-white/50">•</span>
                      <span className="text-sm uppercase tracking-[0.3em] text-white/50">{formatDate(gallery.published_date)}</span>
                    </div>
                    <h2 className="text-2xl font-semibold mb-3 leading-tight text-white">{gallery.title || 'Untitled Gallery'}</h2>
                    <p className="text-white/50 mb-5 leading-relaxed">{gallery.description || 'No description available for this session.'}</p>
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="rounded-3xl bg-white/5 p-4 border border-white/10">
                        ...
                        <p className="mt-2 font-semibold text-white">{gallery.gallery_id}</p>
                      </div>
                      <div className="rounded-3xl bg-white/5 p-4 border border-white/10">
                        <p className="text-xs uppercase tracking-[0.3em] text-gray-500">Status</p>
                        <p className="mt-2 font-semibold text-white">{gallery.is_published ? 'Published' : 'Draft'}</p>
                      </div>
                    </div>
                    <div className="mt-6 flex flex-wrap items-center gap-3">
                      ...
                      <span className="text-sm text-white/50">{gallery.created_by ? `Curated by user ${gallery.created_by}` : 'Curated by the studio'}</span>
                    </div>
                  </div>
                </article>
              ))}
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {fallbackPortfolioImages.map((src, index) => (
                <article key={index} className="rounded-[1.75rem] border border-orange-400/20 bg-white/95 overflow-hidden shadow-xl">
